  ...
  module Control #:nodoc:
    ...
    class Context
      ...
      def source(name, configuration={}, definition={})
        if configuration[:type]
          ...
          when String, Symbol
            source_class = OpenMedia::ETL::Control::Source.class_for_name(configuration[:type])
            sources << source_class.new(self, configuration, definition)
          else
            if configuration[:type].is_a?(OpenMedia::ETL::Control::Source)
              sources << configuration[:type]
            else
              raise ControlError, "Type must be a Class, String, Symbol or object extending OpenMedia::ETL::Control::Source"
            end
          end
        else
          source_types.each do |source_type|
            if configuration[source_type]
              source_class = OpenMedia::ETL::Control::Source.class_for_name(source_type)
              sources << source_class.new(self, configuration, definition)
              break
            end
          end
      ...
      def destination(name, configuration={}, mapping={})
        if configuration[:type]
          ...
          when String, Symbol
            dest_class = OpenMedia::ETL::Control::Destination.class_for_name(configuration[:type])
            destinations << dest_class.new(self, configuration, mapping)
          else
            if configuration[:type].is_a?(OpenMedia::ETL::Control::Destination)
              destinations << configuration[:type]
            else
              raise ControlError, "Type must be a Class, String, Symbol or object extending OpenMedia::ETL::Control::Destination"
            end
          end
        else
          destination_types.each do |dest_type|
            if configuration[dest_type]
              dest_class = OpenMedia::ETL::Control::Destination.class_for_name(dest_type)
              destinations << dest_class.new(self, configuration, mapping)
              break
            end
          end
      ...
      def transform(name, transformer=nil, configuration={}, &block)
        if transformer
          ...
          when String, Symbol
            ...
            begin
              transform_class = OpenMedia::ETL::Transform.const_get(class_name)
          ...
          when Class
            ...
            if transformer.is_a?(OpenMedia::ETL::Transform::Transform)
          ...
          transforms << OpenMedia::ETL::Transform::BlockTransform.new(self, name, :block => block)
        else
      ...
      def define_processor(name, processor_collection, configuration, proc)
        ...
        when String, Symbol, nil
          ...
          begin
            processor_class = OpenMedia::ETL::Processor.const_get(class_name)
            if name == 'block'
    ...
    class Control
      ...
      class << self
        ...
        def parse_text(text)
          control = OpenMedia::ETL::Control::Control.new(nil)
          eval(text, Context.create(control), 'inline')
          control.validate
          control
        end
        
        # Resolve the given object to an OpenMedia::ETL::Control::Control instance. Acceptable arguments
        # are:
        # * The path to a control file as a String
        # * A File object referencing the control file
        # * The OpenMedia::ETL::Control::Control object (which will just be returned)
        #
