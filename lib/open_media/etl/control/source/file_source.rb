  ...
  module Control #:nodoc:
    ...
    class FileSource < Source
      ...
      def each
        ...
        @parser.each do |row|
          if OpenMedia::ETL::Engine.offset && count < OpenMedia::ETL::Engine.offset
            count += 1
          else
            row = OpenMedia::ETL::Row[row]
            row.source = self
            yield row
          end
        end
      ...
      def configure
        ...
        when String, Symbol
          @parser = OpenMedia::ETL::Parser::Parser.class_for_name(configuration[:parser]).new(self)
        when Hash
          name = configuration[:parser][:name]
          options = configuration[:parser][:options]
          @parser = OpenMedia::ETL::Parser::Parser.class_for_name(name).new(self, options)
        else
