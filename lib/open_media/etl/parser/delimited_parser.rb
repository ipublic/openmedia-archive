module OpenMedia::ETL #:nodoc:
  module Parser #:nodoc:
    ...
    class DelimitedParser < OpenMedia::ETL::Parser::Parser
      ...
      def each
        Dir.glob(file).each do |file|
          OpenMedia::ETL::Engine.logger.debug "parsing #{file}"
          line = 0
          lines_skipped = 0
          FasterCSV.foreach(file, options) do |raw_row|
            if lines_skipped < source.skip_lines
              OpenMedia::ETL::Engine.logger.debug "skipping line"
              lines_skipped += 1
              next
            end
            line += 1
      ...
      def validate_row(row, line, file)
        OpenMedia::ETL::Engine.logger.debug "validating line #{line} in file #{file}"
