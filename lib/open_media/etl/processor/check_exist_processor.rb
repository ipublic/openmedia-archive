module OpenMedia::ETL #:nodoc:
  module Processor #:nodoc:
    ...
    # target table
    class CheckExistProcessor < OpenMedia::ETL::Processor::RowProcessor
      ...
      def initialize(control, configuration)
        ...
        @target = configuration[:target] || raise(OpenMedia::ETL::ControlError, "target must be specified")
        @table = configuration[:table] || raise(OpenMedia::ETL::ControlError, "table must be specified")
        @columns = configuration[:columns]
        
        q = "SELECT COUNT(*) FROM #{table_name}"
        @should_check = OpenMedia::ETL::Engine.connection(target).select_value(q).to_i > 0 
      end
