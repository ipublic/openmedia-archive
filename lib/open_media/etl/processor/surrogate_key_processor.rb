module OpenMedia::ETL #:nodoc:
  module Processor #:nodoc:
    ...
    class SurrogateKeyProcessor < OpenMedia::ETL::Processor::RowProcessor
      ...
      def initialize(control, configuration)
        ...
        if table
          @surrogate_key = OpenMedia::ETL::Engine.connection(target).select_value("SELECT max(#{column}) FROM #{table_name}")
        end
