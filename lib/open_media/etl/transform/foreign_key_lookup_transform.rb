module OpenMedia::ETL #:nodoc:
  module Transform #:nodoc:
    ...
    class ForeignKeyLookupTransform < OpenMedia::ETL::Transform::Transform
      ...
      def initialize(control, name, configuration={})
        ...
        if configuration[:cache] ||= true
          if resolver.respond_to?(:load_cache)
            ...
            OpenMedia::ETL::Engine.logger.info "#{resolver.class.name} does not support caching"
          end
        end
      end
      ...
      def transform(name, value, row)
        ...
        unless fk
          ...
          fk ||= @default
          raise ResolverError, "Unable to resolve #{value} to foreign key for #{name} in row #{OpenMedia::ETL::Engine.rows_read}. You may want to specify a :default value." unless fk
          @collection[value] = fk
        end
        fk
      end
...
class SQLResolver
  ...
  # referencing a connection defined in the OpenMedia::ETL database.yml file or an actual
  ...
  def initialize(table, field, connection=nil)
    @table = table
    @field = field
    @connection = (connection.respond_to?(:quote) ? connection : OpenMedia::ETL::Engine.connection(connection)) if connection
  ...
  def table_name
    OpenMedia::ETL::Engine.table(@table, @connection)
  end
  def cache
    @cache ||= {}
  end
